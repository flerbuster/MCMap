...
/**
 * @param {string} text
 * @param {string} mappings_text
 * @returns {string}
 */
function apply_mappings_from_mappings_text(text, mappings_text) {
    let deferred3_0;
    let deferred3_1;
    try {
        const ptr0 = passStringToWasm0(text, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        const ptr1 = passStringToWasm0(mappings_text, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len1 = WASM_VECTOR_LEN;
        const ret = wasm.apply_mappings_from_mappings_text(ptr0, len0, ptr1, len1);
        deferred3_0 = ret[0];
        deferred3_1 = ret[1];
 ...
 * @returns {string}
 */
function file_to_serialized_mappings(file_path) {
    let deferred2_0;
    let deferred2_1;
    try {
        const ptr0 = passStringToWasm0(file_path, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.file_to_serialized_mappings(ptr0, len0);
        deferred2_0 = ret[0];
        deferred2_1 = ret[1];
        return getStringFromWasm0(ret[0], ret[1]);
    } finally {
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('pkg/hello_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
...
const exported = {
    ...
    env: {
        memory: new WebAssembly.Memory({ initial: 1, maximum: 1, shared: false })
    }
}
